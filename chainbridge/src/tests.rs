use super::{
    mock::{
        assert_events, new_test_ext, Balances, Bridge, RuntimeCall, RuntimeEvent, RuntimeOrigin, ProposalLifetime,
...
fn whitelist_chain() {
    new_test_ext().execute_with(|| {
        ...
        assert_events(vec![RuntimeEvent::Bridge(ChainBridgeEvent::ChainWhitelisted(0))]);
    })
}
...
fn asset_transfer_success() {
    new_test_ext().execute_with(|| {
        ...
        assert_events(vec![RuntimeEvent::Bridge(ChainBridgeEvent::NonFungibleTransfer(
            dest_id.clone(),
            2,
            resource_id.clone(),
            token_id,
        ...
        assert_events(vec![RuntimeEvent::Bridge(ChainBridgeEvent::GenericTransfer(
            dest_id.clone(),
            3,
            resource_id,
            metadata,
...
fn asset_transfer_invalid_chain() {
    new_test_ext().execute_with(|| {
        ...
        assert_events(vec![RuntimeEvent::Bridge(ChainBridgeEvent::ChainWhitelisted(
            chain_id.clone(),
        ))]);
